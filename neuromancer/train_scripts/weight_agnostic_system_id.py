"""
Do 100 random runs for each model configuration calculate mean performance and save best model.
We could rank the population by Pareto dominance as from the paper.
They did 80% of the time selecting via mean-performance and number of connections (we can use # parameters)
    and 20% time mean performance and max performance.
...
def logging(args, logtype='mlflow'):
    if logtype == 'mlflow':
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        trainer = Trainer(model, dataset, optimizer, logger=logger, visualizer=visualizer,
                          simulator=simulator, epochs=1, eval_metric=args.eval_metric)
        best_model = trainer.train()
        output = trainer.evaluate(best_model)
        logger.clean_up()
        eval_metrics.append(output[args.eval_metric])
        if output[args.eval_metric] < meta_best_performance:
            meta_best_model = best_model
            meta_best_output = output
            meta_best_performance = output[args.eval_metric]
            metalogger.log_artifacts(dict())

    metalogger.log_parameters()
    metalogger.log_weights(meta_best_model)
    metalogger.log_metrics({**output, f'meta_best_{args.eval_metric}': meta_best_performance,
                            f'meta_mean_{args.eval_metric}': np.mean(eval_metrics)})
    metalogger.cleanup()
