...
def compute_grid(
    ...
    X = X.numpy()
    Y = Y.numpy()
    grid = torch.tensor(np.stack((X.flatten(), Y.flatten())).T, dtype=torch.float)
    ...
    for i, sample in enumerate(grid):
        ...
        if use_bias:
            ...
            Xplus[i, ...] = torch.matmul(sample, Astar)
    """
    Astars, bstars, *_ = lpv_batched(model, grid, use_bias=use_bias)
    if use_bias:
        Xplus = torch.bmm(grid.unsqueeze(1), Astars).squeeze(1) + bstars
    else:
        Xplus = torch.bmm(grid.unsqueeze(1), Astars).squeeze(1)
...
def plot_model_streamlines(
    ...
    mag = np.linalg.norm(np.stack([U, V]), axis=0, ord=1)
