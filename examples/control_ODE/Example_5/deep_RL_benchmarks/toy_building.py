...
def parse_args():
    ...
    opt_group.add_argument('-alg', type=str, choices=['PPO2', 'A2C', 'ACKTR'], default='PPO2')
...
if __name__ == '__main__':
    args = parse_args()
    # os.mkdir(args.savedir)   # TODO: uncomment this for dispatch
    ...
    best_mse = np.finfo(np.float32).max
    best_model = model
    best_model.save(os.path.join(args.savedir, 'RL_model_best.h5'))
    ...
    for i in range(args.epochs):
        ...
        if i % args.verbosity == 0:
            print(f'{i}')
            print(f'{i} '.join([f'{dset}_openmse: {mse:.4f}\t' for dset, mse in zip(['train', 'dev'], mses)]))
        env.reset()
        if args.logger in ['mlflow', 'wandb']:
            mlflow.log_metrics({'train_openmse': mses[0].item(),
                                'dev_openmse':  mses[1].item()}, step=i)
