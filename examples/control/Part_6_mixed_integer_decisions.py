...
if __name__ == "__main__":
    ...
    if STE_method == 'sigmoid':
        # differentiable nearest integer rounding via Sigmoid STE
        def relaxed_round(x, slope=10.0):
            # fractional value with rounding threshold
            backward = (x-torch.floor(x)-0.5)
    ...
    def dynamics(x, u, d): return x @ A.T + u @ B.T + d @ E.T  # plant model
    ...
    preview_steps = 5
    # vector of feasible integers
    integers = torch.arange(u_delta_min, u_delta_max+.1,
    ...
    """
    dynamics_node = Node(
        dynamics, ['x', 'u', 'd'], ['x'], name='dynamics_model'
    )  # system dynamics
    ...
    input_preview_map = {
        "r": {
            "past": 0, "future": preview_steps, "pad_mode": "reflect"
        },
        "d": {
            "past": 0, "future": preview_steps, "pad_mode": "reflect"
        }
    ...
    continous_policy_node = Node(
        net_continous, ['x', 'r', 'd'], ['u_c'], name='continous_input_policy',
        input_map=input_preview_map
    )
    
    integer_policy_node = Node(
        net_integer, ['x', 'r', 'd'], ['u_delta'], name='integer_input_policy',
        input_map=input_preview_map
    )
    ...
    if STE_method == 'sigmoid':
        slope = 10
        def round_fn(u_c, u_delta_relaxed): return torch.cat((u_c, torch.clip(
            relaxed_round(u_delta_relaxed, slope=slope), u_delta_min, u_delta_max)), dim=-1)

    elif STE_method == 'softmax':
        tau = 2  # temperature coefficient in (0,inf)
        def round_fn(u_c, u_delta_relaxed): return torch.cat((u_c, gumbel_softmax(u_delta_relaxed, tau=tau, hard=True) @ integers.T), dim=-1)

    rounding_node = Node(
        round_fn, input_keys=['u_c', 'u_delta'], output_keys=['u'], name='soft_rounding'
    )
    ...
    dist_data_torch = torch.tensor(
        dist_data, dtype=torch.get_default_dtype()).reshape(num_data, nsteps+1, nd)
    # sample references
    ref1_baseline, ref2_baseline = 4, 2  # reference baseline
    ref1_data = ref1_baseline + 1.5 * \
        np.sin(0.05*np.arange(num_data*(nsteps+1))).reshape(-1, 1)
    ref2_data = ref2_baseline + 0.5 * \
        np.cos(0.05*np.arange(num_data*(nsteps+1))).reshape(-1, 1)
    ref_data = np.concatenate((ref1_data, ref2_data), axis=-1)
    ref_data_torch = torch.tensor(
        ref_data, dtype=torch.get_default_dtype()).reshape(num_data, nsteps+1, nref)
    ...
    train_data = DictDataset(
        {
            'x': x_data[:num_train_data], 
            'd': dist_data_torch[:num_train_data],
            'r': ref_data_torch[:num_train_data]
        }, 
        name='train',
    )  # Split conditions into train and dev
    ...
    train_loader = torch.utils.data.DataLoader(
        train_data, batch_size=batch_size, collate_fn=train_data.collate_fn
    )
    dev_loader = torch.utils.data.DataLoader(
        dev_data, batch_size=batch_size, collate_fn=dev_data.collate_fn
    )
    ...
    # reference tracking and control smoothing weights
    Q_control, R_smoothing = 10., 1.  

    tracking_loss = Q_control * ((x == r) ^ 2)     # reference tracking loss

    # control smoothing loss
    input_smoothing_loss = R_smoothing * ((u[:, 1:, :] == u[:, :-1, :]) ^ 2)  
    ...
    optimizer = torch.optim.Adam(
        cl_system.parameters(), lr=0.0003, amsgrad=False, weight_decay=0.
    )
    trainer = Trainer(
        problem,
    ...
    # disturbances
    dists = np.random.beta(0.2, 1., (s_length+nsteps)) * dist_magnitude  
    dists_torch = torch.tensor(
        dists, requires_grad=False, dtype=torch.get_default_dtype()
    ).reshape(1, -1, nd)

    # references
    ref1 = ref1_baseline + 1. * np.sin(0.1*np.arange(s_length+nsteps)).reshape(-1, 1)  
    ref2 = ref2_baseline + 1. * np.cos(0.1*np.arange(s_length+nsteps)).reshape(-1, 1)
    refs = np.concatenate((ref1, ref2), axis=-1)
    refs_torch = torch.tensor(
        refs, requires_grad=False, dtype=torch.get_default_dtype()).reshape(1, -1, nref)

    sim_data = {
        'x': 3*torch.rand(1, 1, nx),
        'r': refs_torch,
        'd': dists_torch
    }
    ...
    ax[0, 0].axhline(x1_min, linestyle=':', c='k', label='bounds')
    ax[0, 0].axhline(x1_max, linestyle=':', c='k')
    ax[0, 0].plot(trajectories['r'][0, :s_length, 0], 'k--', label='reference')
    ax[0, 0].plot(trajectories['x'][0, :, 0], label='process')
    ax[1, 0].plot(trajectories['x'][0, :, 1])
    ax[1, 0].plot(trajectories['r'][0, :s_length, 1], 'k--')
    [ax[1, 0].axhline(i, linestyle=':', c='k') for i in [x2_min, x2_max]]
    [ax[0, 1].axhline(i, linestyle=':', c='k') for i in [u_c_min, u_c_max]]
    ax[0, 1].plot(trajectories['u'][0, :, 0])
    [ax[1, 1].axhline(i, linestyle=':', c='k')
     for i in [u_delta_min, u_delta_max]]
    ax[1, 1].step(np.arange(0, len(trajectories['u'][0, :, 1])),
                  trajectories['u'][0, :, 1], linewidth=1.5)

    [(i.grid(), i.set_xlabel("time"), i.legend()) for i in ax.flat]
    ax[0, 0].set_ylabel('$x_1$')
    ax[1, 0].set_ylabel('$x_2$')
    ax[0, 1].set_ylabel('continous input')
    ax[1, 1].set_ylabel('integer input')
    plt.show()

    plt.figure(figsize=(5, 2))
    plt.plot(trajectories['d'][0, :s_length, 0])
