...
if __name__ == "__main__":
    ...
    list_refs = [np.random.rand(1, nref)*np.ones([args.nsteps, 1]) for k in range(repeats)]
    ...
    # control input constraints dealt with internally by the ControlODE class
    u = variable('u')
    ...
    control_ode = ode.ControlODE(policy=policy, ode=two_tank_ode,
                                 nx=nx, nu=nu, np=nref, u_con=u_constraints)
