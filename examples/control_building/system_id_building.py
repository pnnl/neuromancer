...
def arg_sys_id_problem(prefix='', system='Reno_ROM40'):
    """
    ...
    gp.add("-Q_dy", type=float, default=20.0,                 # tuned: 3
           help="Output tracking derivative penalty weight")
...
if __name__ == "__main__":
    ...
    system = 'Reno_ROM40'       # keyword of selected system
    ...
    if args.downsample != 0:
        data['Y'] = data['Y'][::args.downsample, :]
        data['U'] = data['U'][::args.downsample, :]
        data['D'] = data['D'][::args.downsample, :]
        data['X'] = data['X'][::args.downsample, :]
    ...
    # fx = slim.maps['softSVD'](nx, nx, linargs=linargs)
    ...
    # fd = blocks.MLP(nd, nx, hsizes=[60, 60]) if nu != 0 else None

    dynamics_model = dynamics.BlockSSM(fx, fy, fu=fu, fd=fd, name='dynamics', xou=torch.add,
    ...
    objectives = [regularization, reference_loss, derivative_loss, estimator_loss]
    constraints = [
        state_smoothing, output_smoothing,
