%% Stochastic Model Predictive Control for Simple Building Model
...
% required number of samples to satisty the constraints with probability e
m = 50;
nvars = nx*(m+1)+nu*m+nd+1+nd;
e = 0.1; % 90% constraints satisfaction probability
beta = 1e-3; % probability of achieving e
M = 1+(nvars+1+log(1/beta)+sqrt(2*(nvars+1)*log(1/beta)))/e;
M = round(0.01*M);

%  Probability distribtion of the ambient temperature differences
Text = disturbances(1,:)'; % ambient temperature
Ts_mult = 1;
[dy1 , dx1] = hist(diff(Text(1:Ts_mult:end)),30);
[sigma1,mu1,Adis1]=mygaussfit(dx1,dy1) ;

T = (1:length(Text))'*Ts/3600/24/Ts_mult;
% temperature profile
  figure
  plot(T(1:Ts_mult:end),Text(1:Ts_mult:end),'LineWidth',2)
 axis tight
 title('External temperature');
 xlabel('time [days]');
 ylabel('temperature [^{\circ}C]');
grid on
%   temperature differences in each sampling instant
  figure
  plot(T(1:Ts_mult:end-Ts_mult),diff(Text(1:Ts_mult:end)),'LineWidth',2)
   axis tight
 title('External temperature differences');
 xlabel('time [days]');
 ylabel('temperature difference [^{\circ}C]');
grid on
% histogram and gauss fit to data
  y=Adis1.*exp( -(dx1-mu1).^2 / (2*sigma1^2) );
  figure
   hist(diff(Text(1:Ts_mult:end)),30)
   h = findobj(gca,'Type','patch');
   h.FaceColor = [0.4 0.4 0.9];
%   histogram(diff(Text(1:Ts_mult:end)),30)
  hold on
  plot(dx1,y,'r','LineWidth',2)
     title('Histogram of external temperature differences');
 ...
 ylabel('number of samples [-]');
  ...
  figure
  plot(T(1:Ts_mult:end-Ts_mult),diff(Internal(1:Ts_mult:end)),'LineWidth',2)
   axis tight
 title('External temperature differences');
 xlabel('time [days]');
 ylabel('temperature difference [^{\circ}C]');
grid on
% histogram and gauss fit to data
  y=Adis3.*exp( -(dx3-mu3).^2 / (2*sigma3^2) );
  figure
   hist(diff(Internal(1:Ts_mult:end)),30)
   h = findobj(gca,'Type','patch');
   h.FaceColor = [0.4 0.4 0.9];
%   histogram(diff(Text(1:Ts_mult:end)),30)
  hold on
  plot(dx3,y,'r','LineWidth',2)
     title('Histogram of external temperature differences');
 xlabel('temperature difference [^{\circ}C]');
 ylabel('number of samples [-]');
  axis tight
...
%     f = find(dist_gy1 >= 0.9);
%     dist_gx(f(1))
    dist1 = normrnd(mu,sigma/4); %4, 0.25
    D1 = [D1, dist1];
end
%  stochastic disturbances on ambient temperature for constraints tightenint
% d_max = zeros(nd,1);
% d_min = zeros(nd,1);
d_max(1) = max(D1);
d_min(1) = min(D1);

%% Stochastic MPC formulation via Yalmip
%------------------------------------------------------
% constraints values
umin = 0;
umax = 5000;
