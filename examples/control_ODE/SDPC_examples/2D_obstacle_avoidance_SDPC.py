"""
Solve stochastic obstacle avoidance control problem in Neuromancer
minimize     u_k^2
subject to   (p/2)^2 <= b(x[0]-c)^2 + (x[1]-d)^2
             x_k+1 = A(V)x_k + Bu_k + w_k

problem parameters:             p, b, c, d
problem decition variables:     x, u
problem uncertainties:          V (parametric), w (additive)
...
def plot_obstacle(model, args, b, c, d, w_sigma=0.005, show_plot=True, show_plot_loss=False):
    """
    ...
    for j in range(w_runs):
        ...
        start_time = time.time()
        xout = dynamics_model({**uout, 'x0_estimator': x_torch, 'Yf': Yf, 'wf': wf})
    ...
    if show_plot:
        ...
        # plot DPC state trajectories
        for j in range(w_runs):
            if j==0:
                ax.plot(X_trajs[j][:, 0], X_trajs[j][:, 1], '-', color='purple', label='DPC', linewidth=0.5)
            else:
                ax.plot(X_trajs[j][:, 0], X_trajs[j][:, 1], '-', color='purple', linewidth=0.5)
        # plot IPOPT trajectory
        ax.plot(X_ipopt[:, 0], X_ipopt[:, 1], '-', color='cyan', label='IPOPT', linewidth=2)
...
if __name__ == "__main__":
    ...
    sequences = {
        ...
        "Y": np.repeat(np.random.uniform(low=-0.1, high=0.1, size=(n_sim_p, nx)),
                       n_sim_w, axis=0),
        "x_final": np.repeat(np.random.uniform(low=[0.8, -0.2], high=[1.2, 0.2],
                                               size=(n_sim_p, nx)), n_sim_w, axis=0),
        'params': np.repeat(np.random.uniform(low=[1.0, 0.3, 0.0], high=[3.0, 0.7, 0.4],
                                              size=(n_sim_p, 3)), n_sim_w, axis=0),
        # "w": np.random.uniform(low=-0.05, high=0.05, size=(nsim, nx)),   # additive uncertainties
    ...
    dynamics_model.fd.linear.weight = torch.nn.Parameter(torch.eye(nx, nw))
    ...
    # constraints and objective names
    obstacle.name = 'obstacle'
    action_loss.name = 'min(u)'
    reference_loss.name = 'target'
    state_smoothing.name = 'x_smooth'
    control_smoothing.name = 'u_smooth'
    state_lower_bound_penalty.name = 'x_min'
    state_upper_bound_penalty.name = 'x_max'
    input_lower_bound_penalty.name = 'u_min'
    input_upper_bound_penalty.name = 'u_max'
    ...
    sol_time, _ = plot_obstacle(problem, args, b, c, d, w_sigma=w_scale*w_sigma)
    p, b, c, d = 0.5, 3.0, 0.3, 0.3
    sol_time, _ = plot_obstacle(problem, args, b, c, d, w_sigma=w_scale*w_sigma)
    p, b, c, d = 0.5, 2.0, 0.3, 0.1
    sol_time, _ = plot_obstacle(problem, args, b, c, d, w_sigma=w_scale*w_sigma)
    ...
    times_ipopt = []
    for k in range(50):
        sol_time, sol_time_casadi = plot_obstacle(problem, args,
