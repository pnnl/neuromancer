...
stub = 'python system_id.py -gpu 0 -epochs 15000 -location %s -logger mlflow -warmup 100 -patience 100 ' % args.results
...
for j in params.index:
    ...
    for i in range(5):
        for ab in ablations:
            ...
            argstring = ' '.join(['-' + p + ' ' + str(params_copy[p][j]) for p in params_copy.columns] + [' -savedir %s ' % savedir])
            cmd = stub + argstring
            print(cmd)
