...
def plot_ode_phase_portrait(
    ...
    if len(initial_states) > 0:
        ...
        ax.plot(states[:, 0], states[:, 1], ms=3)
...
def plot_ode_streamlines(
    ...
    mag = np.linalg.norm(np.stack([Xp0, Xp1]), axis=0, ord=1)
    ...
    if len(initial_states) > 0:
        ...
        for n in range(initial_states.shape[0]):
            for i in range(t):
                states[i + 1, n, :] = ode.simulate(nsim=1, x0=states[i, n, :], U=None)["Y"][0, :]
        states = np.transpose(states, axes=(0, 2, 1))
        ax.plot(states[:, 0], states[:, 1], ms=3, alpha=0.7)
...
if __name__ == "__main__":
    ...
    plot_ode_streamlines(
        ...
        initial_states=np.array(
            [
                [1 + np.cos(x), np.sin(x)]
                for x in np.arange(0.25, 2 * np.pi, 2 * np.pi)
            ]
        ) * 0.1
    )
    plt.show()
