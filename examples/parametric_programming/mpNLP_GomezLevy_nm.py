"""
Solve the Gomez and Levy function problem, formulated as the NLP using Neuromancer toolbox:
minimize     4*x**2 - 2.1*x**4 + 1/3*x**6 + x*y - 4*y**2 + 4*y**4
...
if __name__ == "__main__":
    ...
    samples = {"p": np.random.uniform(low=0.5, high=2.0, size=(nsim, 1))}
    ...
    def NLP_param(p):
        ...
        opti.minimize(4 * x ** 2 - 2.1 * x ** 4 + 1 / 3 * x ** 6 +
                      x * y - 4 * y ** 2 + 4 * y ** 4)
        opti.subject_to(x >= y)
    ...
    J = 4*xx**2 - 2.1*xx**4 + 1/3*xx**6 + xx*yy - 4*yy**2 + 4*yy**4
    c1 = xx - yy
    c2 = xx ** 2 + yy ** 2 - (p / 2) ** 2
    c3 = -(xx ** 2 + yy ** 2) + p ** 2

    fig, ax = plt.subplots(1, 1)
    levels = [-5, -1., -0.5, -0.1, 0.0, 0.1, 0.5, 1, 2, 3, 4, 5, 7, 10, 20, 50,
    ...
    def eval_objective(x, y):
        obj_value_mean = np.mean(4*x**2 - 2.1 * x**4 + 1/3 * x**6 + x*y - 4*y**2 + 4*y**4)
