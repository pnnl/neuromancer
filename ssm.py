...
class BlockSSM(nn.Module):
    def __init__(self, nx, ny, nu, nd, bias=False, f_x=Linear, f_u=Linear,
                 f_d=Linear, f_y=Linear, xou=torch.add, xod=torch.add,
                 f_x_args=dict(), f_u_args=dict(), f_d_args=dict(), f_y_args=dict()):
        """
...
#       TODO: select here model parametrizations - how to do this in a generic way?
        self.f_x = f_x(nx, nx, bias=bias, **f_x_args)
        self.f_u = f_u(nu, nx, bias=bias, **f_u_args)
        self.f_d = f_d(nd, nx, bias=bias, **f_d_args)
        self.f_y = f_y(nx, ny, bias=bias, **f_y_args)
...
class MLPHeatFlow(nn.Module):
    ...
    def __call__(self, u):
        return self.nlin(self.layer2(self.nlin(self.layer1(u))))
