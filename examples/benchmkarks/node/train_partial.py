...
class Validator:
    ...
    def __init__(self, netG, sys, args, stats=None, normalize=False, scaled=False):
        """
        ...
        :param stats: (dict: {str: torch.tensor}) This is a dictionary with mean, min, max, std, var for X and U
        ...
        for x0 in self.x0s:
            u = sys.get_U(1000)
            sim = sys.simulate(ts=args.ts, nsim=1000, x0=x0, U=u)
            X.append(torch.tensor(sim['Y'][:-1, :], dtype=torch.float32))
            U.append(torch.tensor(sim['U'], dtype=torch.float32))
            if normalize:
                X[-1] = (X[-1] - stats['xmean']) / stats['xstd']
                U[-1] = (U[-1] - stats['umean']) / stats['ustd']

        def mse_loss_scaled(input, target):
            return torch.mean(((input - target) ** 2) / stats['xvar'].view(1, 1, -1), axis=(1, 2))
    ...
    def __call__(self):
        ...
        with torch.no_grad():
            ...
            if self.normalize:
                x = x * self.stats['xstd'] + self.stats['xmean']
                xprime = xprime * self.stats['xstd'] + self.stats['xmean']
...
def get_data(nsteps, sys, nsim, bs, stats=None, normalize=False):
    """
    :param nsteps: (int) Number of timesteps for each batch of training data
    :param sys: (psl.ODE_NonAutonomous)
    :param stats: (dict {str: torch.Tensor}) Dictionary with mean, std, and var of X and U
    :param normalize: (bool) Whether to normalize the data

    """
    X, T, U = [], [], []
    for _ in range(nsim):
        sim = sys.simulate(nsim=nsteps, x0=sys.get_x0(), U=sys.get_U(nsteps))
        X.append(sim['Y'][:-1, :])
        U.append(sim['U'])

    X, U = np.stack(X), np.stack(U)
    sim = sys.simulate(nsim=nsim * nsteps, x0=sys.get_x0(), U=sys.get_U(nsim * nsteps))

    sim['Y'] = sim['Y'][:-1, :]
    nx, nu = X.shape[-1], U.shape[-1]
    x, u = sim['Y'].reshape(nsim, nsteps, nx), sim['U'].reshape(nsim, nsteps, nu)
    X, U = np.concatenate([X, x], axis=0), np.concatenate([U, u], axis=0)

    Xstats, Ustats = X.reshape(-1, nx), U.reshape(-1, nu)
    if stats is None:
        stats = {'xmean': torch.tensor(Xstats.mean(axis=0, keepdims=True, dtype=np.float32)),
                 'xstd': torch.tensor(Xstats.std(axis=0, keepdims=True, dtype=np.float32)),
                 'xvar': torch.tensor(Xstats.var(axis=0, keepdims=True, dtype=np.float32)),
                 'umean': torch.tensor(Ustats.mean(axis=0, keepdims=True, dtype=np.float32)),
                 'ustd': torch.tensor(Ustats.std(axis=0, keepdims=True, dtype=np.float32))}
    X = torch.tensor(X, dtype=torch.float32)
    U = torch.tensor(U, dtype=torch.float32)
    if normalize:
            X = (X - stats['xmean']) / stats['xstd']
            U = (U - stats['umean']) / stats['ustd']
...
# building_systems_linear = {k: partialclass(k, True) for k in building_systems}
# building_systems_nonlinear = {k: partialclass(k, False) for k in building_systems}

systems = {**systems, **building_systems_linear, **building_systems_nonlinear}
# functools.partial(scipy.integrate.odeint, tfirst=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-model', type=str, choices=[k for k in get_model.keys()], default='koopman',
    ...
    def mse_loss(input, target):
        ...
        """
        return torch.mean(((input - target) ** 2) / stats['xvar'].view(1, 1, -1))
    ...
    if args.normalize:
        pred_traj = (pred_traj * stats['xstd'] + stats['xmean'])
        true_traj = (X * stats['xstd'] + stats['xmean'])
