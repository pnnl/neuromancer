...
if __name__ == "__main__":
    SEED = 419
    outdir = "spectral_distributions_20210527"
    ...
    combos = [
        ...
        (("gershgorin", 1.2, 1.4, True), 1, [("gelu", False)]),
        (("gershgorin", 1.2, 1.4, True), 4, [("gelu", False)]),
        (("gershgorin", 1.2, 1.4, True), 8, [("gelu", False)]),
    ]
    ...
    for i, ((linmap, sigmin, sigmax, real), nlayers, params) in enumerate(combos):
        ...
        if i == len(combos) - 1 or (i < len(combos) - 1 and combos[i][0] != combos[i+1][0]):
            ...
            for nl, eigval_list in zip(plot_nlayers, eigvals):
                ...
                real = np.real(eigval_list)
                #imag = np.imag(eigval_list)
                sns.kdeplot(real, log_scale=True, label=f"Layers = {nl}", )
                #sns.kdeplot(imag, log_scale=True, label=f"Complex, Layers = {nl}")
            plt.xlim(1e-6, 1000)
            plt.tight_layout()
            plt.legend()
